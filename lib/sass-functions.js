...
module.exports = function factory(options) {
	...
	const ratio = options.divide ? 2 : 1;
	...
	function resolve(file) {
		...
		const absolute = options.resolve(file);
		if (absolute) {
			pathMap[file] = absolute;
			return absolute;
		}
		return null;
	}
	...
	function width(name) {
		...
		return new SassNumber(size.width / ratio, 'px');
	}
	...
	function height(name) {
		...
		return new SassNumber(size.height / ratio, 'px');
	}
