...
function parseOptions(opt) {
	...
	if (typeof backgroundImage === 'function') {
		...
		options.style['background-image'] = function calcBackgroundImage(data) {
			const publicUrl = options.path.public(data.meta);
			if (options.isRetina) {
				const retinaPublicUrl = publicUrl.replace(/\.png$/, '@2x.png');
				const fallbackUrl = options.isDivided ? publicUrl : retinaPublicUrl;
				return [
	...
	if (backgroundPosition === 'percent') {
		options.style['background-position'] = function calcBackgroundPositionPercent(data) {
			...
			/* eslint-enable no-mixed-operators, prefer-template */
			const str = `${x} ${y}`;
		...
		const ratio = options.isDivided && options.isRetina ? -2 : -1;
		options.style['background-position'] = function calcBackgroundPosition(data) {
			const coordinate = data.coordinate;
			const str = String(coordinate.x / ratio).concat('px ', coordinate.y / ratio, 'px');
			const minify = str.replace(/(^|\s)-?0px/g, '$10');
			return minify;
		};
	}
	if (typeof backgroundSize === 'function') {
		options.style['background-size'] = backgroundSize.bind(null);
	} else {
		const ratio = options.isDivided && options.isRetina ? 2 : 1;
		options.style['background-size'] = function calcBackgroundSize(data) {
			const properties = data.properties;
			return String(properties.width / ratio).concat('px ', properties.height / ratio, 'px');
		};
	}
